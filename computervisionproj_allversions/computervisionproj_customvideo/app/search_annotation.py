...
def search_annotation():
    ...
    try:
        ...
        # Check if 'timestamp' column exists to differentiate between video and image CSVs
        if 'timestamp' in df.columns and 'frame' in df.columns:
            # Collect bounding box data with timestamps and frames for videos
            box_data = matched_boxes[['timestamp', 'frame', 'bounding_boxes']].to_dict(orient='records')
        else:
            # Collect bounding box data for images
