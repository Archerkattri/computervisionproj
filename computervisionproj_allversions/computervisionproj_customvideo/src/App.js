...
function App() {
  ...
  const [timestamps, setTimestamps] = useState([]); // Added for video processing
  ...
  const handleClear = () => {
    ...
    setAnnotations([]);
    setBoundingBoxes([]);
    setTimestamps([]); // Clear timestamps
  };
  ...
  const handleAnnotationClick = async (annotation) => {
    ...
    try {
      ...
      if (selectedType === 'image') {
        ...
        const boxes = searchResponse.data.boxes;

        // Transform the boxes data
        const annotations = boxes.map(box => ({
          bounding_boxes: JSON.parse(box.bounding_boxes),
          frame: box.frame,
          timestamp: box.timestamp
        }));

        // Clear previously set annotated video URL before setting a new one
        setAnnotatedFileUrl(null);

        // Generate annotated video
        const generateVideoResponse = await axios.post('http://localhost:5000/generate-video', {
          video_name: `${file.name}`, // Path to the original video
          annotations: annotations
        });
        console.debug('Generate Video Response: ' + JSON.stringify(generateVideoResponse.data));

        // Set URL for annotated video
        const uniqueId = new Date().getTime(); // Generate a unique ID based on the current timestamp
